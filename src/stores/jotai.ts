...
export function useLogsStreamReader() {
    ...
    const previousKey = usePreviousDistinct(
        `${apiInfo.server}/logs?level=${level}&secret=${apiInfo.secret}`
    );
    ...
    useEffect(() => {
        if (level) {
            ...
            const logUrl = `${apiInfo.server.replace(
                /^http/,
                "ws"
            )}/logs?level=${level}&token=${apiInfo.secret}`;
            item.connect(logUrl);
        }
...
export function useConnectionStreamReader() {
    ...
    const connection = useRef(
        new StreamReader<Snapshot>({ bufferLength: 200 })
    );

    const url = `${apiInfo.server.replace(/^http/, "ws")}/connections?token=${
        apiInfo.secret
    }`;
