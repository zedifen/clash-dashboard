...
export const languageAtom = atomWithStorage<Lang | undefined>(
    'language',
    undefined
)
        ...
        set(
            result.isErr()
                ...
                : {
                      ...
                      premium: !!result.value.data.premium,
                  }
        )
    })
...
export function useRuleProviders() {
    ...
    const { data, mutate } = useSWR(
        ...
        async () => {
            ...
            ).map<API.RuleProvider>(
                (name) => ruleProviders.data.providers[name]
            )
        }
    )
...
export function useProxy() {
    ...
    const { mutate } = useSWR(['/proxies', client], async () => {
        ...
        const [proxy, groups] = partition(
            proxies,
            (proxy) => !policyGroup.has(proxy.type)
        )
        set({
            proxies: proxy as API.Proxy[],
            groups: groups as API.Group[],
            global,
        })
    })

    const markProxySelected = useCallback(
...
export function useLogsStreamReader() {
    ...
    const previousKey = usePreviousDistinct(
        `${apiInfo.server}/logs?level=${level}&secret=${encodeURIComponent(
            apiInfo.secret
        )}`
    )
    ...
    useEffect(() => {
        if (level) {
            ...
            const logUrl = `${apiInfo.server.replace(
                /^http/,
                'ws'
            )}/logs?level=${level}&token=${encodeURIComponent(apiInfo.secret)}`
            item.connect(logUrl)
        }
...
export function useConnectionStreamReader() {
    ...
    const url = `${apiInfo.server.replace(
        /^http/,
        'ws'
    )}/connections?token=${encodeURIComponent(apiInfo.secret)}`
